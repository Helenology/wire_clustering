...
# sklearn
from sklearn.cluster import AgglomerativeClustering as sklearnHC

# cuML-related libraries
import cudf
import cuml
import cupy as cp
from cuml.cluster import AgglomerativeClustering as cumlHC
...
# NOTE: Hierarchical clustering has O(n^2) memory complexity for the distance matrix.
# For n_samples=100000, this requires ~80GB RAM. If memory is insufficient,
# consider reducing n_samples (e.g., to 10000).
...
print(f"Starting HC (Agglomerative) Benchmark with {n_replicates} replicates...")
...
_ = sklearnHC(n_clusters=n_clusters).fit_predict(X_warm_cpu)
_ = cumlHC(n_clusters=n_clusters).fit_predict(X_warm_gpu)
...
for i in range(n_replicates):
    ...
    sk_hc = sklearnHC(n_clusters=n_clusters)
    sk_labels = sk_hc.fit_predict(X_cpu)
    ...
    # 2. cuML (GPU)
    start_time = time.time()
    cu_hc = cumlHC(n_clusters=n_clusters)
    cu_labels = cu_hc.fit_predict(X_gpu)
    ...
    output_filename = "python_hc_results.csv"
...
print("\n" + "="*50)
print(f"    HC Benchmark Summary ({n_replicates} Replicates)    ")
print("-" * 50)
